...
export default async function DashboardPage() {
  ...
  const [monitor, page, corridors, exceptions, tenant] = await Promise.all([
    ...
    api.listShipments({ limit: 8 }).catch(() => null),
    api.corridors().catch(() => []),
  ...
  const active =
    (monitor?.entities.shipmentsByStatus ?? [])
      ...
      .reduce((sum, r) => sum + r.n, 0) || (page?.total ?? 0);
  const inTransit = byStatus.get("IN_TRANSIT") ?? 0;
  const customsHold = byStatus.get("CUSTOMS") ?? 0;

  // Lanes come from a database-side aggregate over the whole book, not from
  // the page of rows below — a dashboard map that silently described only the
  // first page would be worse than no map at all. A lane goes critical when
  // any shipment on it has an open exception, which makes the map a triage
  // surface rather than decoration.
  const lanes: CorridorLane[] = corridors.map((c) => ({
    origin: c.origin,
    destination: c.destination,
    value: c.n,
    tone: c.exceptions > 0 ? "critical" : "accent",
  }));
    ...
    <div>
      ...
      <Panel className="mb-4">
        <PanelHeader
          ...
          eyebrow="Freight on the water and on the road"
          actions={
            lanes.some((l) => l.tone === "critical") ? (
              <Tag tone="critical" dot>
                Lane with open exception
              </Tag>
            ) : undefined
          }
        />
        <CorridorMap
          lanes={lanes}
          aspect={2.9}
          emptyLabel="Nothing in transit — the map fills in as shipments are booked."
        />
      </Panel>
